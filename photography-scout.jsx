...
const SUPABASE_URL = "https://uwgmfxxcphetsxgwxhmu.supabase.co";
const SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InV3Z21meHhjcGhldHN4Z3d4aG11Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzE0MzIyMzcsImV4cCI6MjA4NzAwODIzN30.PsSy4wT17PD-938KZiDILkRhx9oq_nSP2m2A2CkZu_Q";
const GMAPS_KEY    = "AIzaSyCQ-WY9HZqhISj0ibTtRHzyCnzVGgHf8A4";
const EBIRD_KEY    = "4q96frd4qg57";
...
2 sentences per section max. Use <ul><li> for any lists. Honest about poor conditions. No camera settings.`;
    ...
    try {
      const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:MODEL,max_tokens:1200,messages:[{role:"user",content:prompt}]})});
...
Answer Matt's questions with specific, actionable advice tailored to his Peninsula. Reference his actual sightings data where relevant. Be direct and concise — he's a professional, skip the basics. Use HTML for structure if helpful (h4, p tags). Keep responses under 300 words.`;
    ...
    try {
      const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:MODEL,max_tokens:600,system:systemPrompt,messages:[{role:"user",content:userMsg}]})});
      const d=await res.json();
