...
${focusLoc && ML_LOCATION_INTEL[focusLoc.name] ? `MACAULAY LIBRARY SPECIES INTELLIGENCE (${ML_LOCATION_INTEL[focusLoc.name].totalRecords} records at this location):
Peak species: ${ML_LOCATION_INTEL[focusLoc.name].topSpecies.slice(0,6).map(s=>s.name+(s.peakMonths.includes(date.getMonth()+1)?' [PEAK NOW]':'')).join(', ')}
` : ""}
